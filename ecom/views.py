...
from intasend import APIService, Collect
...
class PaymentService:
    def __init__(self):
        self.auth_token = "ISSecretKey_test_52a19eb9-8c8d-4976-9fe2-b38a7ab10cdd"
        self.collect = Collect(token=self.auth_token)

    def get_access_token(self, request):
        publishable_key = 'ISPubKey_test_441b87b1-5740-45a7-9b24-722df4355ed9'
        service = APIService(token=self.auth_token, publishable_key=publishable_key, test=True)
        return service

    def lipa_na_mpesa_online(self, request, phone_number, email, amount):
        narrative = "Payment for items in cart"
        service = self.get_access_token(request)
        response = service.collect.mpesa_stk_push(phone_number=phone_number,
                                                  email=email, amount=amount, narrative=narrative)
        print(response)
        if response.get('status') == 'success':
            return redirect('payment_success')
        else:return redirect('payment_success')
...
def customer_address_view(request):
    product_ids = request.COOKIES.get('product_ids', '')
    product_in_cart = bool(product_ids)
    product_count_in_cart = len(set(product_ids.split('|'))) if product_ids else 0

    addressForm = forms.AddressForm(request.POST or None)
    if addressForm.is_valid():
        email = addressForm.cleaned_data['Email']
        mobile=addressForm.cleaned_data['Mobile']
        address = addressForm.cleaned_data['Address']
        total = sum(p.price for p in models.Product.objects.filter(id__in=product_ids.split('|'))) if product_ids else 0

        payment_service = PaymentService()
        response = payment_service.lipa_na_mpesa_online(request, mobile, email, total)
